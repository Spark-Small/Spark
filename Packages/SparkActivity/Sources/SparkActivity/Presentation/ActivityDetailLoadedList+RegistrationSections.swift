...
extension ActivityDetailLoadedList {
    @ViewBuilder
    func registrationScrollSection(activity: ActivityDetail, showsRSVPButtons: Bool) -> some View {
        VStack(alignment: .leading, spacing: SparkLayoutMetrics.compactVerticalPadding) {
            meetupDetailSubsectionHeader(ActivityRSVPStatus.invited.registrationSectionTitle)
            ...
            meetupInsetActionsGroup {
                ...
                if showsRSVPButtons {
                    rsvpScrollButtons(for: activity)
                }
    ...
    @ViewBuilder
    func registrantActionsScrollSection(activity: ActivityDetail) -> some View {
    ...
    @ViewBuilder
    func groupChatScrollSection(activity: ActivityDetail) -> some View {
    ...
    @ViewBuilder
    private func rsvpScrollButtons(for activity: ActivityDetail) -> some View {
        ForEach(ActivityRSVPStatus.selectableResponses, id: \.rawValue) { status in
            ...
            if status != ActivityRSVPStatus.selectableResponses.last {
                meetupActionDivider()
            }
        }
    }
    ...
    @ViewBuilder
    func registrationSection(activity: ActivityDetail, showsRSVPButtons: Bool) -> some View {
        Section {
            if let blocked = activity.registrationBlockedMessage {
                Text(blocked)
                    .font(.footnote)
